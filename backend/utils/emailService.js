...
const sendEmail = async ({ to, subject, htmlContent }) => {
  try {
    ...
    const response = await fetch("https://api.brevo.com/v3/smtp/email", {
      ...
      body: JSON.stringify({
        sender: { name: "PlaceMate", email: SENDER_EMAIL },
        to,
        subject,
        htmlContent,
      }),
...
const otpTemplate = (title, name, otp, message) => `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 10px; text-align: center;">
        ...
        <p style="font-size: 12px; color: #888888;">&copy; 2026 PlaceMate. All rights reserved.</p>
    </div>
`;
...
export const sendVerificationEmail = async (email, name, otp) => {
  return sendEmail({
    to: [{ email, name }],
    subject: "Your verification code - PlaceMate",
    htmlContent: otpTemplate(
      "Verify your Email",
      name,
      otp,
      "Thank you for signing up. Plese use the following 6-digts code to verify your email",
    ),
  });
};
...
export const sendForgotPasswordEmail = async (email, name, otp) => {
  return sendEmail({
    to: [{ email, name }],
    subject: "Reset your password - PlaceMate",
    htmlContent: otpTemplate(
      "Reset your password",
      name,
      otp,
      "You requested to reset your password. Please use the followig 6-digits code to proceed.",
    ),
  });
};
...
export const sendAdminInquiryEmail = async (data) => {
  const htmlContent = `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 10px;">
            ...
            <p>You have received a new inquiry from the PlaceMate contact form.</p>
            ...
            <hr style="border: 0; border-top: 1px solid #eeeeee; margin: 20px 0;">
            <p style="font-size: 12px; color: #888888; text-align: center;">This is an automated notification from PlaceMate.</p>
        </div>
    `;
  return sendEmail({
